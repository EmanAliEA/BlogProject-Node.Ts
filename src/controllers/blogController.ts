...
const getBlogById = async (req: Request, res: Response) => {
    ...
    if (!checkOwnership(blog, (req as any).user)) return res.status(400).send('you are not allowed to update this blog');
    return blog;
}
// update Blog
const updateBlog = async (req: Request, res: Response) => {
    const blog = getBlogById(req, res);
    (blog as any)?.updateOne(req.body);
    return res
        .status(200)
